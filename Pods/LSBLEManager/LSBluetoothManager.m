...
/* Bước sử dụng Bluetooth: 1, 2 quét cho Bluetooth, kết nối Bluetooth 3, 4 phát hiện dịch vụ Bluetooth, tính năng phát hiện (Channel) 
 Gói dịch vụ này có năm cái bắt tay (kết nối) 6, đặc trưng bởi việc sử dụng gửi hoặc nhận tin nhắn  */
 ...
 *  Trạng thái di động Bluetooth 0 Unknown 1, thiết lập lại 2, 3 không hỗ trợ bất hợp pháp 4, 5 đóng cửa, mở
 */
...
- (void)scanDevice{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(_instance.centralManager.state == CBManagerStatePoweredOn){
            NSLog(@"Tìm kiếm thiết bị bluetooth");
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary<NSString *,id> *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    NSLog(@"Các thiết bị bluetooth được tìm thấy %@",peripheral);
...
- (void)stopScanDevice{
    ...
    NSLog(@"Dừng tìm kiếm các thiết bị bluetooth");
    if (_instance.centralManager) {
...
- (void)reConnectDevice{
    ...
    NSLog(@"Bắt đầu kết nối các thiết bị bluetooth");
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    if (error) {
        NSLog(@"Phát hiện ra： %@ lỗi: %@", peripheral.name, [error localizedDescription]);
        [_instance updateBLEStatue:BLESTATUE_CHARACT_FAIL];
        return;
    }
    
    NSLog(@"Phát hiện ra：%@",service.characteristics);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    if (error) {
        NSLog(@"Lỗi nhận dữ liệu：%@",[error localizedDescription]);
        return;
    }
    
    NSLog(@"Dữ liệu nhận được：%@",characteristic.value);
...
- (void)updateBLEStatue:(BLESTATUE)statue{
    ...
    NSLog(@"Tình trạng bluetoth %d", (int)_instance.bleStatue);
}
